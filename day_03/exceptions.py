# Define error contract here - one place, one shape, forever
...
def register_exception_handlers(app):
    ...
    async def book_not_dound_handler(request: Request, exc: BookNotFoundError):
    ...
    # 2. Handle DuplicateISBNError -> 409 Conflict
    ...
    # 3. Override FastAPI's default 422 validation error format or Pydantic validation errors
    @app.exception_handler(RequestValidationError)
    async def validation_exception_handler(request: Request, exc: RequestValidationError):
        request_id = getattr(request.state, "request_id", "unknown")
        # Reformat Pydantic's verbose errors into something cleaner
        details = [
            {
    ...
    # 4. Handle HTTPException (404s, 403s etc. raised explicitly in handlers)
    ...
    # 5. Catch-all for unhandled exceptions (500s)
